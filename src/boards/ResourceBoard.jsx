  ...
  return (
    <div className="bg-white bg-opacity-70 rounded-lg">
      {rows.map((row, rowIndex) => (
        <div key={rowIndex} className="flex gap-3 pl-1 p-2 ml-2 mr-2 mt-2 ">
          {row.map((res) => {
            let imgSrc = "";
            // 동적 자원(wood, clay, stone, food, grain, sheep)
            if (
              ["wood", "clay", "stone", "food", "grain", "sheep"].includes(
                res.key
              )
            ) {
            ...
            return (
              <div key={res.key} className="flex items-center gap-2">
                ...
                <span className="font-medium text-green-900">{res.count}</span>
              </div>
            );
          })}
        </div>
